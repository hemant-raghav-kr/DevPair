...
interface OwnerApplicationCardProps {
  ...
  onStatusUpdate: (appId: string, newStatus: "accepted" | "rejected" | "removed") => Promise<void> | void;
  isUpdating?: boolean;
}
...
export function OwnerApplicationCard({
  ...
  return (
    <>
          ...
          {application.status === "accepted" && (
            <div className="flex items-center gap-2 self-end sm:self-auto">
              <button
                ...
                className="px-3 py-1.5 rounded-xl border border-red-200 dark:border-red-900/60 bg-red-50/50 dark:bg-red-950/30 text-red-600 dark:text-red-400 font-semibold hover:bg-red-100/70 dark:hover:bg-red-900/50 transition-colors disabled:opacity-50 text-xs"
              >
                Remove Member
              </button>
            </div>
      ...
      {/* Confirmation Modal for Kick / Member Removal */}
      {isRemoveModalOpen && (
        <RemoveMemberModal
          isOpen={isRemoveModalOpen}
          onClose={() => setIsRemoveModalOpen(false)}
          applicationId={application.id}
          memberName={applicant.full_name || applicant.username}
          projectTitle={projectTitle}
          roleTitle={application.role?.title}
          onSuccess={() => {
            onStatusUpdate(application.id, "removed");
          }}
        />
      )}
    </>
